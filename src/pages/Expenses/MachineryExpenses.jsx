  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/api/MachineryExpenses/MachineryExpenses",
        formData
      );

      if (response.data.success) {
        alert("MachineryExpenses expense submitted successfully");
        handleReset();
      } else {
        alert("Failed to submit MachineryExpenses expense");
      }
    } catch (error) {
      console.error("Error submitting MachineryExpenses expense:", error);
